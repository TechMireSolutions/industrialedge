...
export default function Home() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [
          ...
          cmsApi.getHomepageSections ? cmsApi.getHomepageSections() : Promise.resolve({ data: { data: [] } })
        ])
        ...
        let sectionsData = sectionsResponse.data?.data || sectionsResponse.data || []
        // Sort by order
        if (Array.isArray(sectionsData)) {
          sectionsData.sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0))
        } else {
          sectionsData = []
        }
  ...
  const renderSection = (section) => {
    if (section.active === false || section.enabled === false) return null;
    
    switch (section.type || section.sectionName) {
      case 'Hero':
        return (
          <div key={section.id} className="container-fluid hero-section px-0 position-relative overflow-hidden" data-theme="dark">
            <HeroBackground />
      ...
      case 'Partners':
        if (settings?.featureFlags?.enablePartners === false || !partners || partners.length === 0) return null;
        return (
          <div key={section.id} className="container-fluid py-4 bg-white text-center" data-theme="light">
      ...
      case 'Products':
        return (
          <div key={section.id} className="container-fluid py-6 bg-white" data-theme="light">
            <div className="container py-5">
              <div className="text-center mb-5 wow fadeInUp" data-wow-delay="0.1s">
                <span className="text-uppercase fw-bold text-muted letter-spacing-2">{section.subtitle || 'Curated Collection'}</span>
                <h2 className="display-4 fw-bold text-primary mt-2">{section.title || 'Featured Products'}</h2>
      ...
      case 'Banners':
        ...
        return (
          <div key={section.id} className="container-fluid py-5" style={{ backgroundColor: '#f8f9fa' }} data-theme="light">
      ...
      case 'Services':
        return (
          <div key={section.id} data-theme="light">
      ...
      case 'CTA':
        return (
          <div key={section.id} className="container-fluid py-6 position-relative overflow-hidden" style={{ backgroundColor: 'var(--primary-color)' }} data-theme="dark">
  ...
  return (
    <div className="home-page-container">
      ...
      {sections.length > 0 ? (
        ...
        <div className="container-fluid py-5 text-center text-muted" data-theme="light">
          <p className="mb-0">No homepage sections configured. Please set them up in the Admin Panel.</p>
        </div>
      )}
    </div>
  )
